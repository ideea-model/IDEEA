...
if (packageVersion("IDEEA") < "0.0.36") {
  stop('Please update IDEEA
       pak::pkg_install("ideea-model/IDEEA@dev")')
}
stopifnot(packageVersion("IDEEA") >= "0.0.36")
```
...
```{r, warning=FALSE}
transmission_existing <- ideea_data$transmission[[regN]] |>
  filter(case == "current", !is.na(MW)) |>
  rename(dst = region1) |>
  filter(MW > 0)
...
network <- transmission_existing |>
  filter(case == "current") |>
  mutate(
    trd_name = paste("TRL", region, dst, sep = "_")
  ) |>
  rename(
    region.x = region,
    region.y = dst
...
scen_REF_sub@path <- file.path(
  "D:/Dropbox/projects/IDEEA_scenarios", glue("REF_sub_r{nreg}"))
...
scen_REF_sub <- save_scenario(scen_REF_sub, path = "scenarios/REF_sub")
...
# store scenario on disk (unload from RAM)
scen_CAP_sub <- save_scenario(scen_CAP_sub, path = "scenarios/CAP_sub")
...
getData(scen_CAP_sub, "pDemand", digits = 1, drop.zeros = T, merge = T) |> 
  group_by(name, comm) |> 
  summarize(GWh = sum(value))
...
vTechCap_agg |> st_drop_geometry() |> group_by(scenario, process, year) |>
  summarise(GW = sum(GW)) |>
...
## Trade
```{r, eval=params$solve}
vTradeCap <- getData(sns, "vTradeCap", merge = T)

b <- a |>
  left_join(network, by = c("region" = "region.x", dst = "region.y",
                            "case" = "case", MW = "MW")) |>
