...
if (packageVersion("energyRt") < "0.50") {
  stop('Please update energyRt.
       pak::pkg_install("energyRt/energyRt@dev")')
}
if (packageVersion("IDEEA") < "0.50") {
  stop('Please update IDEEA
       pak::pkg_install("ideea-model/IDEEA@dev")')
}
stopifnot(packageVersion("IDEEA") >= "0.50")
```

## Configuration

Set the base year (BY), the number of regions (5 or 32), and other spatial configuration.
```{r, eval = TRUE}
...
# https://web.stanford.edu/group/efmh/jacobson/Articles/I/WindSpacing.pdf
# https://www.nrel.gov/docs/fy09osti/45834.pdf, p.18
...
# see: https://www.nrel.gov/docs/fy19osti/72399.pdf, p.18
# convert("ha/MW", "km2/MW", 40) 
```


Adjust wind and solar capacity factors clustering tolerance (from 0 to 1). The clustering process is based on correlation of the capacity factors between MERRA2 cells (see details on [merra2ools](https://energyrt.github.io/merra2ools/articles/merra2ools.html#cluster-locations). The lower value gives more clusters per region, and larger regions' area will have more clusters and may require higher values to reduce the number of clusters. 
...
ideea_modules$calendars$calendar_d365_h24@timetable
```
...
```{r, fig.cap="Hourly load by region, 2019 (first six regions)"}
...
```{r, fig.cap="Solar clusters and average solar capacity factors by cluster, tilted tracking system (tl)", echo=FALSE}
# plot solar clusters
fig_1 <-ggplot() +
  # geom_sf(data = ideea_sf, fill = "grey") +
  geom_sf(aes(fill = as.factor(cluster)), 
          data = ideea_sol_cl_sf, color = NA) +
  geom_sf(data = ideea_sf, fill = NA, color = "grey") +
  scale_fill_viridis_d(option = "C", name = "cluster") +
  theme_ideea_map()

# plot the solar capacity factors
fig_2 <- ggplot() +
  geom_sf(data = ideea_sf, fill = "grey") +
  geom_sf(aes(fill = scf_tl), data = ideea_sol_cl_sf, color = NA) +
  geom_sf(data = ideea_sf, fill = NA, color = "grey") +
  scale_fill_viridis_c(option = "B") +
  theme_ideea_map()

cowplot::plot_grid(fig_1, fig_2, nrow = 1)
```
...
```{r, fig.cap="Wind clusters and average wind capacity factors by cluster, 100m height", echo=FALSE}
# plot wind clusters
fig_1 <- ggplot() +
  geom_sf(data = ideea_sf, fill = "grey90")

if (win_clust_max <= 10) {
  fig_1 <- fig_1 +
    geom_sf(aes(fill = as.factor(cluster)),
            data = ideea_win_cl_sf, color = NA) +
    scale_fill_viridis_d(option = "C", name = "cluster")
} else {
  fig_1 <- fig_1 +
    geom_sf(aes(fill = cluster),
            data = ideea_win_cl_sf, color = NA) +
    scale_fill_viridis_c(option = "C", name = "cluster")
}
fig_1 <- fig_1 + theme_ideea_map()

# plot the wind capacity factors
fig_2 <- ggplot() +
  geom_sf(data = ideea_sf, fill = "grey90") +
  geom_sf(aes(fill = wcf_100m), data = ideea_win_cl_sf, color = NA) +
  geom_sf(data = ideea_sf, fill = NA, color = "grey") +
...
electricity <- list(
  ...
  ESOL_w_const = ESOL_w_const, # solar generators with capacity constraints
  ESOL_w_land = ESOL_w_land, # solar generators with land constraints
  EWIN_w_const = EWIN_w_const, # onshore wind generators w/capacity constraints
  EWIN_w_land = EWIN_w_land, # onshore wind generators w/land constraints
  EWIF_w_const = EWIF_w_const, # offshore wind generators w/capacity constraints
  EWIF_w_land = EWIF_w_land, # offshore wind generators w/land constraints
  ...
  # transmission
  repo_transmission_ac = repo_transmission_ac, # transmission network
  repo_transmission_dc = repo_transmission_dc, # transmission network
  # unserved load
  ...
  CO2_CAP = CO2_CAP
  # calendars
  # calendar_d365_h24 = calendar_d365_h24, # full calendar
...
scen_REF_sub <- solve(scen_REF_sub, wait = T, tmp.del = F)
