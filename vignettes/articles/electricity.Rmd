...
if (packageVersion("IDEEA") < '0.0.33') {
  stop('Please update IDEEA
       pak::pkg_install("ideea-model/IDEEA@dev")')
}
stopifnot(packageVersion("IDEEA") >= '0.0.33')
...
```{r}
solar_cf <- get_ideea_cf("sol", nreg = nreg, year = 2019, tol = .03)
solar_cf$cluster |> unique()

name_with_cluster <- function(name, cluster, ndigits = 2) {
  paste0(
    name,
    formatC(cluster, digits = ndigits, width = ndigits, 
            flag = "0", mode = "integer")
    )
}
name_with_cluster("", 0)
...
for (i in unique(solar_cf$cluster)) {
  ...
  WSOL_i <- newWeather(
    name = name_with_cluster("WSOL", i),
    desc = name_with_cluster(
      "Solar capacity factors, tilted tracking system (tl), cluster ",
      i
    ),
...
# repository to store capacity factors (weather objects)
WWIN <- newRepository(name = "Wind capacity factors")

for (i in unique(wind_cf$cluster)) {
  x <- filter(wind_cf, cluster == i)
  WWIN_i <- newWeather(
    name = name_with_cluster("WWIN", i),
    desc = name_with_cluster(
...
for (tec in ideea_modules$techs$ESOL@data) { # vintages
  for (w in WSOL@data) { # CF by clusters
    # stop() # use for debug
    cl <- str_extract(w@name, "[0-9]+$") |> as.integer()
    # update prototype technology
    tec_w <- update(
      tec,
      name = name_with_cluster(paste0(tec@name, "_"), cl),
      weather = list(
        weather = w@name,
        waf.fx = 1
      ),
...
# update base-year technology with preexisting capacity
ESOL@data$ESOL_2020_01 <- ESOL@data$ESOL_2020_01 |>
  update(capacity = data.frame(
    region = cap_sol[[regN]],
    year = cap_sol$year,
...
#     tech,
#     weather = data.frame(
#       # assuming that the location is in the first cluster
#       # (can be changed later using geo-location of existing plants)
#       weather = name_with_cluster("WSOL", 1), 
#       waf.fx = 1
#       ),
...
# update base-year technology with preexisting capacity
# (assigning to the first cluster, geo-location can be matched later)
EWIN@data$EWIN_2020_01 <- EWIN@data$EWIN_2020_01 |>
  update(capacity = data.frame(
    region = cap_win[[regN]],
    year = cap_win$year,
...
# })
names(EWIN)
EWIN@data$EWIN_2020_01@weather
EWIN@data$EWIN_2020_01 |> draw()
```
...
# Reference case
scen_base_sub <- interpolate_model(
  mod,
  name = "BASE_sub",
