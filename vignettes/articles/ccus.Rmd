...
# the objects is declared in `energy` document and saved in ideea_modules$energy
# here we import saved objects for the requested number of regions

# A commodity to represent the storage of CO2
CO2SINK <- ideea_modules$energy[[regN]]$CO2SINK

# Permanent geological carbon storage (saline aquifers and basalt) potential
RES_CO2SINK <- ideea_modules$energy[[regN]]$RES_CO2SINK
  
# Declaration of captured CO2 "commodity" 
CCSCO2 <- newCommodity(
  name = "CCSCO2", 
...
```{r, echo=FALSE}
kableExtra::kable(ideea_modules$energy[[regN]]$COA@emis) |>
  kableExtra::kable_styling(full_width = F)
```
and for natural gas (`GAS@emis`):
```{r, echo=FALSE}
kableExtra::kable(ideea_modules$energy[[regN]]$GAS@emis) |>
  kableExtra::kable_styling(full_width = F)
```
...
```{r}
COA <- ideea_modules$energy[[regN]]$COA
...
# Efficiency, no CCS
kableExtra::kable(
  select(ECOA_prototype@ceff, region, year, comm, cinp2use)
)

# Efficiency, with CCS (~10% loss)
...
```{r, eval=TRUE}
GAS <- ideea_modules$energy[[regN]]$GAS
...
# saving the module
# !!! Important: building of the module depends on output from energy.Rmd
# !!! When updating after energy.Rmd, make sure that the results have been
# !!! saved to the package data: run the 'energy' chunk of DATASET.R 
# !!! and rebuild the package before processing the CCUS.
repo_CCUS <- newRepository(
  name = "CCS Technologies",
  desc = "Repository for CC(U)S technologies",
  # CO2SINK, RES_CO2SINK, # saved in energy.Rmd by regN
  CCSCO2, GHG,
...
if (isTRUE(params$save_ccus_objects)) {
  fname <- glue("data-raw/repo_CCUS.RData")
  message("Saving: ", fname)
  save(repo_CCUS, file = here(fname))
}

```

```{r, eval=FALSE, include=FALSE}
# testing - old version, not updated
library(data.table)
