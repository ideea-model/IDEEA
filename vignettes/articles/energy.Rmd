...
coa_sup <- 
  get_ideea_data("coal", nreg = 7, "production", agg_fun = sum) |>
  full_join(
    get_ideea_data("coal", nreg = 7, "cost_USD_t_2020", agg_fun = mean)
  )
...
oil_sup <- get_ideea_data(name = "oil", nreg = 7, variable = "reserve") |>
  full_join(
...
SUP_OIL <- newSupply(
  ...
  commodity = "OIL",
  unit = "GWh",
  region = unique(oil_sup$region),
  reserve = data.frame(
    region = oil_sup$region,
...
oil_sf <- get_ideea_map(nreg = 7, rename = T, islands = T) |> 
  left_join(oil_sup)
...
oil_r36_sf <- get_ideea_map(nreg = 36, rename = TRUE) |> 
...
gas_reserve <- get_ideea_data(name = "gas", nreg = 7, 
                              variable = "reserve", agg_fun = sum)
# gas_sup <- 
#   # get_ideea_data("oil", nreg = 7, "production") |>
#   gas_reserve |>
#   full_join(
#     get_ideea_data("gas", nreg = 7, variable = "cost", 
#                    agg_fun = mean)
#   ) |>
#   filter(gas_reserve_GWh_2021 > 0)

# gas_ava <- get_ideea_data(name = "gas_ava_assumptions", nreg = 7)
# gas_sup |>
#   full_join(ideea_data$gas_ava_assumption, by = c(reg7 = "region"))
...
SUP_GAS <- newSupply(
  ...
  commodity = "GAS",
  unit = "GWh",
  region = unique(gas_reserve$region),
  reserve = data.frame(
    region = gas_reserve$region,
    res.up = gas_reserve$gas_reserve_GWh_2021 
  ) |> unique(),
  availability = ideea_data$gas_ava_assumption
)
...
gas_sf <- get_ideea_map(nreg = 7, rename = T, islands = T) |> 
  left_join(gas_reserve)
...
gas_r36_sf <- get_ideea_map(nreg = 36, rename = T) |> 
...
# loading and processing ccs data
get_ideea_data(name = "ccs_r5", raw = T) # check raw data
ccs_reserve <- get_ideea_data(name = "ccs_r5", raw = T)

# Declaration of commodity
CCSCO2 <- newCommodity(
  name = "CCSCO2", 
  desc = "Captured CO2, kt",
  unit = "kt", 
  #slice = "ANNUAL", deprecated
  timeframe = "ANNUAL"
)
...
GHG <- newCommodity(
  name = "GHG",
  desc = "Greenhouse gas emissions, Mt",
  unit = "Mt",
  agg = data.frame(
    comm = c("CO2", "CCSCO2"),
    unit = "kt",
  ...
  misc = list(
    info = c("GHG is a composite commodity that aggregates CO2 and CCSCO2 with",
             "specified weights (in @agg$agg)")
  )
)
...
# Saving the model objects in repository
repo_energy <- add(repo_energy, CCSCO2, CO2SINK, GHG, RES_CO2SINK, 
                   overwrite = T)
repo_energy@data |> names()
```
</details>
...
bio_ava <- ideea_data$biomass_ava_assumption 
...
RES_BIO <- newSupply(
  ...
  commodity = "BIO",
  unit = "GWh",
  availability = bio_ava,
  region = unique(bio_ava$region)
)
...
bio_sf <- get_ideea_map(nreg = 7, rename = T, islands = T) |> 
  left_join(bio_sup)
...
bio_r36_sf <- get_ideea_map(nreg = 36, rename = T) |> 
