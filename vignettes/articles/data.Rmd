...
#   mutate(
#     # datetime = tsl2dtm(slice, tmz = "Asia/Kolkata", year = 2019),
#     YDAY = tsl2yday(slice),
#     HOUR = tsl2hour(slice),
#     .after = "slice"
#   )

load_2019_MWh <- get_ideea_data("load_2019_MWh", raw = TRUE) 
kableExtra::kable(head(load_2019_MWh))
...
# get_ideea_data("load_2019_MWh")

load_2019_reg5 <- get_ideea_data("load_2019_MWh", nreg = 5) %>% 
  group_by(region, slice, datetime, YDAY, HOUR) %>%
...
ggplot(load_2019_reg5) +
  geom_line(aes(HOUR, MWh/1e3, color = YDAY, group = YDAY), alpha = .25) +
  scale_color_viridis_c(option = "H", limits = c(1, 365), name = "Day of year") +
  facet_wrap(~region, ncol = 1, scales = "free_y") +
  labs(y = "Load, GWh", x = "Hour of a day") +
  theme_bw()
...
locid_win_cl_k$k |> as.factor() |> summary()
...
locid_win_cl_k_i$k |> as.factor() |> summary()
...
ggplot() +
  ...
  geom_sf(fill = "wheat", data = get_ideea_map(nreg = 34), inherit.aes = F) +
